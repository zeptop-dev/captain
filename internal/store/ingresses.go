...
const ingressCols = "id, node_id, name, kind, bind_ip, line_ip, entry_host, port_from, port_to, port_offset, created_at, updated_at"

func scanIngress(row interface{ Scan(...any) error }) (*Ingress, error) {
	var g Ingress
	var cr, up int64
	if err := row.Scan(&g.ID, &g.NodeID, &g.Name, &g.Kind, &g.BindIP, &g.LineIP, &g.EntryHost, &g.PortFrom, &g.PortTo, &g.PortOffset, &cr, &up); err != nil {
...
func (s *Store) CreateIngress(ctx context.Context, g *Ingress) error {
	...
	ts := now()
	res, err := s.db.ExecContext(ctx, `INSERT INTO ingresses (node_id, name, kind, bind_ip, line_ip, entry_host, port_from, port_to, port_offset, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		g.NodeID, g.Name, g.Kind, g.BindIP, g.LineIP, g.EntryHost, g.PortFrom, g.PortTo, g.PortOffset, ts, ts)
	if err != nil {
		return err
	}
...
func (s *Store) UpdateIngress(ctx context.Context, g *Ingress) error {
	_, err := s.db.ExecContext(ctx, `UPDATE ingresses SET name = ?, kind = ?, bind_ip = ?, line_ip = ?, entry_host = ?, port_from = ?, port_to = ?, port_offset = ?, updated_at = ? WHERE id = ?`,
		g.Name, g.Kind, g.BindIP, g.LineIP, g.EntryHost, g.PortFrom, g.PortTo, g.PortOffset, now(), g.ID)
	return err
}
