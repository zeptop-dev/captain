...
interface ExtNode { id: number; source_id: number | null; name: string; uri: string; group_id: number | null; rate: number; sort: number; enabled: boolean }
...
export default function ExternalPage() {
  ...
  const srcForm = useForm({ initialValues: { name: '', url: '', user_agent: 'v2rayN/7.0', group: '', rate: 1, enabled: true } })
  const saveSrc = useMutation({ mutationFn: (v: typeof srcForm.values) => api.post<{ synced: number; error: string }>('/api/admin/external/sources', { name: v.name, url: v.url, user_agent: v.user_agent, group_id: v.group ? Number(v.group) : null, rate: v.rate, enabled: v.enabled }), onSuccess: (r) => { r.error ? toast.err(new Error(r.error)) : toast.ok(t('external.synced', { count: r.synced })); setAddingSrc(false); srcForm.reset(); invalidate() }, onError: toast.err })
  ...
  return (
    <>
      ...
      <Card p={0}><Table.ScrollContainer minWidth={720}><Table>
        <Table.Thead><Table.Tr><Table.Th>{t('external.name')}</Table.Th><Table.Th>{t('external.link')}</Table.Th><Table.Th>{t('external.from')}</Table.Th><Table.Th>{t('inbounds.group')}</Table.Th><Table.Th>{t('entries.rate')}</Table.Th><Table.Th>{t('common.enabled')}</Table.Th><Table.Th /></Table.Tr></Table.Thead>
        <Table.Tbody>
          {(nodes.data ?? []).map((n) => (
            <Table.Tr key={n.id}>
          ...
          {(nodes.data ?? []).length === 0 && <Table.Tr><Table.Td colSpan={7}><Text c="dimmed" ta="center" py="lg">{t('external.empty')}</Text></Table.Td></Table.Tr>}
        </Table.Tbody>
